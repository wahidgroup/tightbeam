...
//! - Chunked large-frame roundtrip and interleave at the negotiated chunk size
//! - Stream-credit stall at the initial window, resume on a peer grant
//! - Zero session budget refuses data emits, control and drain still work
//! - Budget exhaustion triggers GoAway(BudgetExhausted), owed echo resolves
//! - Flow violations (oversize chunk, credit/budget overrun): GoAway(ProtocolError)
//! - TransportAuthorizer reduced grant lands in both endpoints' settings
//! - TransportAuthorizer refusal aborts the handshake with the app code
//! - Rekey drain with an in-flight chunked transfer (dynamic headroom)
//! - Cleartext parity: chunking and stream credit with budgets inert
...
mod negotiated {
	...
	use tightbeam::transport::envelopes::{
		...
		MuxEnvelope, MuxOpenPackage, MuxPingPackage, MUX_APPLICATION_CODE_FLOOR,
	};
	use tightbeam::transport::handshake::negotiation::{
		AuthorizationGrant, AuthorizationRefusal, MuxBudgets, MuxSettings, NegotiationError, TransportAuthorizer,
		TransportOffer,
	};
	...
	fn large_mux_frame(label: &str) -> Frame {
	...
	fn chunked_offer(cap: u32) -> TransportOffer {
	...
	struct MuxEndpointConfig {
		...
		grantor: Option<Arc<dyn CreditGrantor>>,
	}
	...
	async fn establish_client_mux_server_raw_with(
	...
	/// client halves.
	fn split_server_mux_client_raw(
	...
	async fn establish_server_mux_client_raw_with(
		client_offer: TransportOffer,
		server_offer: TransportOffer,
		server_config: MuxEndpointConfig,
	) -> Result<ServerMuxClientRaw, TightBeamError> {
	...
	fn muxed_request_envelope(stream_id: u32, frame: Frame) -> Result<TransportEnvelope, TightBeamError> {
	...
	fn is_budget_exhausted(result: &Result<Option<Frame>, TransportError>) -> bool {
	...
	async fn read_remaining_chunks(
		reader: &mut SplitReader,
		stream_id: u32,
		mut payload: Vec<u8>,
	...
	async fn read_until_goaway(reader: &mut SplitReader, reason: GoAwayReason) -> Result<bool, TightBeamError> {
	...
	async fn await_goaway_reason(handle: &MuxHandle, reason: GoAwayReason) -> bool {
	...
	const REFUSAL_CODE: u32 = MUX_APPLICATION_CODE_FLOOR;

	/// Authorizer refusing every session with [`REFUSAL_CODE`].
	struct RefusingAuthorizer;
	...
	tb_scenario! {
		...
		environment Bare {
			exec: |SetupEnv { trace, .. }| async move {
				...
				write_muxed_echo(&mut link.server_writer, client_stream_id(0), &Arc::new(ack.clone())).await?;
	...
	async fn credit_overrun_rejected() -> Result<bool, TightBeamError> {
		...
		let open = MuxOpenPackage::new(1, false, chunk.clone())?;
	...
	async fn reassembly_flood_rejected() -> Result<bool, TightBeamError> {
		...
		let envelopes = vec![
			MuxOpenPackage::new(client_stream_id(0), false, partial.clone())?.into(),
			MuxOpenPackage::new(client_stream_id(1), false, partial.clone())?.into(),
			MuxOpenPackage::new(client_stream_id(2), false, partial.clone())?.into(),
			MuxOpenPackage::new(client_stream_id(3), false, partial.clone())?.into(),
