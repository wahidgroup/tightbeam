...
//!
//! ## Attack
//! An attacker replays or splices a budget-bearing key exchange with a
//! forged countersignature, or strips the countersignature attribute
//! from the client Finished. The handshake correctly aborts, but the
//! operator's ledger shows nothing: repeated probes leave no evidence.
//!
//! ## Expected control
//! Every concluded receipt exchange MUST reach the observer before the
//! abort: a failing countersignature records
//! `SessionVerdict::CountersignatureInvalid` (with the bytes that
//! failed), an absent one records
//! `SessionVerdict::CountersignatureMissing`. Settlement MUST never fire
//! and the session MUST never activate.
//!
//! ## References
...
mod ecies {
	...
	use crate::common::security::{
		default_security_profile, expectation_failure, pinning_validator, PayingApprover, RecordingObserver,
	...
	tb_assert_spec! {
		pub CountersignatureInvalidSpec,
		V(1,0,0): {
			mode: Accept,
			gate: Ok,
			assertions: [
				(tampered_countersignature_rejected, exactly!(1), equals!(true)),
				(settle_never_fired, exactly!(1), equals!(true)),
				(outcome_records_invalid_attempt, exactly!(1), equals!(true))
			]
		}
	}

	// A countersignature tampered on the wire (the client auth signature
	// never covered it) MUST abort the handshake, MUST NOT settle, and
	// MUST reach the observer as CountersignatureInvalid evidence.
	tb_scenario! {
		name: ecies_invalid_countersignature_recorded,
		spec: CountersignatureInvalidSpec,
		environment Bare {
			exec: |SetupEnv { trace, .. }| async move {
				...
				// Tamper only the countersignature: the client auth
				// signature covers transcript, ciphertext, and certificate
				// but not this field, so the message still authenticates
				// and the tamper is caught by countersignature
				// verification alone.
				let mut kex = ClientKeyExchange::from_der(&client_kex_der)?;
				let countersignature = kex
					.receipt_signature
					.take()
					.ok_or_else(|| expectation_failure("budget-bearing kex must carry a countersignature"))?;
				let mut forged = countersignature.as_bytes().to_vec();
				let middle = forged.len() / 2;
				forged[middle] ^= 0xFF;
				kex.receipt_signature = Some(OctetString::new(forged)?);
				let tampered = kex.to_der()?;

				let kex_result = server.process_client_key_exchange(&tampered).await;
				let rejected = matches!(kex_result, Err(HandshakeError::SignatureVerificationFailed));
				trace.event_with(
					CountersignatureInvalidSpec::tampered_countersignature_rejected,
					&[],
					rejected,
				)?;

				trace.event_with(
					CountersignatureInvalidSpec::settle_never_fired,
					&[],
					authorizer.settle_calls() == 0,
				)?;

				// The forged attempt is evidence: exactly one outcome, the
				// invalid verdict, and the bytes that failed verification.
				let outcomes = observer.recorded();
				let invalid_recorded = outcomes.len() == 1
					&& outcomes[0].verdict == SessionVerdict::CountersignatureInvalid
					&& outcomes[0].client_signature.is_some();
				trace.event_with(
					CountersignatureInvalidSpec::outcome_records_invalid_attempt,
					&[],
					invalid_recorded,
				)?;
...
mod cms {
	...
	/// Re-encode the client Finished without its `RECEIPT_SIGNATURE`
	/// unsigned attribute. No signature covers unsigned attributes, so
	/// the stripped message stays signature-valid.
	fn strip_receipt_signature(client_finished: &[u8]) -> Result<Vec<u8>, TightBeamError> {
		...
		let retained: Vec<_> = attrs
			.iter()
			.filter(|attribute| attribute.oid != RECEIPT_SIGNATURE)
			.cloned()
			.collect();
	...
	// A client Finished stripped of its countersignature attribute MUST
	// abort the receipt acknowledgement, MUST reach the observer as
	// CountersignatureMissing evidence (with the recovered settlement
	// answer), and MUST NOT activate the session.
	tb_scenario! {
		...
		environment Bare {
			exec: |SetupEnv { trace, .. }| async move {
				...
				// The MITM strips the countersignature on the wire.
				let stripped = strip_receipt_signature(&client_finished)?;
				...
				// The withheld countersignature is evidence: one outcome,
				// the missing verdict, no signature bytes, and the
				// settlement answer recovered from the envelope.
				let outcomes = observer.recorded();
				...
				let missing_recorded = outcomes.len() == 1
					...
					&& outcomes[0].client_signature.is_none()
					&& outcomes[0].ancillary_response == expected_answer;
				trace.event_with(
