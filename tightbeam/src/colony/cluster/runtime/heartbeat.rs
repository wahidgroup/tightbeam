		...
		+ 'static,
{
	...
	pub(crate) fn spawn_heartbeat<D: ClusterDigest>(self) -> rt::JoinHandle {
		let GatewayRuntimeCtx { registry, servlet_registry, config, pool, trace, .. } = self;
	...
	/// Runs the pheromone evaporation and relay-trail pruning loop.
	pub(crate) fn spawn_evaporation(self, relay_trail_ttl: Duration) -> rt::JoinHandle {
		...
		rt::spawn(async move {
			loop {
				...
				// Relay trails refresh only through relayed rumors, so a trail
				// alone leaves an unpicked trail in place, so age is its
				// lifecycle bound. A retired
				// fallback traces with its count, so a route that silently
				// vanished stays diagnosable (ISO 27001 A.8.15).
